...
class Pagimagic extends Component<Props, State> {
  ...
  handleChangeCurrentPageIndex = (pageIndex: number): void => {
    if (this.props.changePageIndex != null) {
  ...
  getItemsPerPage = (): number => this.props.itemsPerPage

  getCurrentPage = (): number => this.state.currentPage

  getMaximumVisiblePaginators = (): number => this.props.maximumVisiblePaginators

  getList = (): any[] => this.props.list

  // where should splice starts
  startList = (): number => this.getCurrentPage() * this.getItemsPerPage()

  // where should splice ends
  endList = (): number => this.startList() + this.getItemsPerPage()

  // elements which should to be shown
  getVisibleList = (): any[] => this.getList().slice(this.startList(), this.endList())

  getMax = (): number => this.getTotalPaginators() > this.getMaximumVisiblePaginators()
    ? this.getMaximumVisiblePaginators()
    : this.getTotalPaginators()

  needToRenderArrows = (): boolean => this.getTotalPaginators() > this.getMax()

  needToShowCounter = (): boolean => Boolean(this.props.showCounter)
  ...
  createIterator = (currentPage: number): PaginationIterator => {
    ...
    const make = (condition: boolean) => (memo: PaginationIterator) => (el: number): void => {
      if (condition) memo.push(el)
    }
    const makeFirst = (memo: PaginationIterator, i: number): void => {
      if (i === 0) memo.push(i)
    }
    const makeEmpty = (condition: boolean) => (memo: PaginationIterator): void => {
      if (this.needToRenderArrows() && condition) memo.push('...')
    }
    const makeLast = (memo: PaginationIterator, i: number): void => {
      if (i + 1 === VISIBLE) memo.push(TOTAL - 1)
    }

    const result = Array.apply(null, Array(VISIBLE)).reduce((memo: PaginationIterator, _, i): PaginationIterator => {
      /**
